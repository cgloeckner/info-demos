...
class TkGrid(ABC):
    ...
    def __init__(self, root: tk.Tk, width: int, height: int, cell_size: int):
        ...
        self._canvas = tk.Canvas(root, width=width * cell_size, height=height * cell_size)
        self._canvas.pack()
    ...
    def _get_shape_callable(self, shape: TkShape) -> Callable[..., Any]:
    ...
    def draw_shapes(self, grid_data: list, shape: TkShape) -> None:
        ...
        shape_callable = self._get_shape_callable(shape)

        index = 0
        for y in range(self.__height):
            y1 = y * self._cell_size
            y2 = (y+1) * self._cell_size
            
            for x in range(self.__width):
                x1 = x * self._cell_size
                x2 = (x+1) * self._cell_size
